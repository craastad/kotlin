...
class BridgeTest : TestCase() {
     ...
     * Also checks that the graph satisfies the following conditions:
     * 1. Each fake override should have a super-declaration
     * 2. Each abstract fake override doesn't have a concrete super-declaration. More accurately, for each abstract fake override F,
     *    each concrete super-declaration reachable from F should be also reachable from some abstract super-declaration, reachable from F.
     * 3. Each concrete fake override should have exactly one concrete super-declaration. More accurately, for each concrete
    ...
    private fun graph(vararg edges: Pair<Fun, Fun>) {
        ...
        for (vertex in vertices.filter { !it.isDeclaration }) {
            ...
            if (vertex.isAbstract) {
                assert(concreteDeclarations.isEmpty(),
                       "Incorrect test data: abstract fake override vertex $vertex has a concrete super-declaration, " +
                       "which is not overridden by any abstract declaration: $concreteDeclarations")
            }
            else {
                assert(!concreteDeclarations.isEmpty(),
    ...
    // Simple tests where declaration "a" is inherited by declaration "b" with a different signature and there should be a bridge.
    // Note that there seems to be no use for the bridge near an abstract declaration, but we generate it anyway to comply with javac
    ...
    fun testAbstractDeclarationOverridesTwoConcreteDeclarations() {
        val a = v("+D1")
        val b = v("+D2")
        val c = v("-D3")
        graph(c to a, c to b)
    ...
    fun testConcreteDeclarationOverridesTwoConcreteDeclarations() {
        val a = v("+D1")
        val b = v("+D2")
        val c = v("+D3")
        graph(c to a, c to b)
    ...
    fun testConcreteDeclarationOverridesTwoInheritingConcreteDeclarations() {
        val a = v("+D1")
        val b = v("+D2")
        val c = v("+D3")
    ...
    fun testDiamondConcreteDeclarations() {
        val a = v("+D1")
        val b = v("+D2")
        val c = v("+D3")
        val d = v("+D4")
    ...
    fun testDiamondConcreteFakeOverridesInTheMiddle() {
        val a = v("+D1")
        val b = v("+F2")
        val c = v("+F3")
        val d = v("+D4")
    ...
    fun testFakeOverrideAbstractAndConcreteInheritingDeclarations() {
        val a = v("-D1")
        val b = v("+D2")
        val c = v("+F3")
        graph(c to a, c to b, b to a)
    ...
    fun testDiamondFakeOverrideConcreteDeclarations() {
        val a = v("+D1")
        val b = v("+F2")
        val c = v("+D3")
        val d = v("+F4")
    ...
    // Fake override overrides another fake override with some bridges already present there

    fun testFakeOverrideInheritsBridgesFromFakeOverride() {
        val a = v("-D1")
        val b = v("+D2")
        val c = v("+F3")
        val d = v("+F4")
    ...
    fun testFakeOverrideInheritsBridgesAndAbstract() {
        ...
        doTest(c, setOf(bridge(a, b)))
        // It's important that "e" shouldn't have "a->b" bridge, since it's inherited from "c"
    ...
    fun testFakeOverrideDiamondWithExtraAbstract() {
        ...
        doTest(c, setOf(bridge(a, c)))
        // It's important that "e"'s implementation is "c", not "a", so a bridge "d->c" should exist
